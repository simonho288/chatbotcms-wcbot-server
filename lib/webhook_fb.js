...
function handleTextMessage(e, t, s, n) {
    ...
    sendTextMessage(r, t, s, e.text)) : e.messageData ? callSendMessageApi(r, e.messageData) : (logger.error(util.format("%s-%s: Unhandled result!", path.basename(__filename), __line)), 
    sendTextMessage(r, t, s, "Internal error: Unhandled result!"))).then(e => {
...
function receivedPostback(e, t) {
    ...
    }).then(e => !1 === e || (null == e ? sendTypingMessage(a, s, !1) : e.text ? sendTextMessage(a, s, n, e.text) : e.messageData ? callSendMessageApi(a, e.messageData) : e.profile ? void callSendProfileApi(a, e.profile) : (logger.error(util.format("%s-%s: Unhandled result!", path.basename(__filename), __line)), 
    sendTextMessage(a, s, n, "Internal error: Unhandled result!")))).then(() => {
...
function getMessengerWhitelistedDomain(e) {
    return new Promise((t, s) => {
        request({
            uri: "https://graph.facebook.com/v2.6/me/messenger_profile",
            qs: {
...
function setupWhitelistedDomains(e, t) {
    let s;
    s = t instanceof Array ? t : [ t ];
    for (let e = 0; e < s.length; ++e) s[e].indexOf("localhost") >= 0 ? s.splice(e--, 1) : "/" != s[e].indexOf(s[e].length - 1) && (s[e] += "/");
    return s.push("https://wcbot.chatbotcms.com/"), s.push("https://chatbotcms.com/"), 
    s.push("https://www.chatbotcms.com/"), new Promise((t, n) => {
        getMessengerWhitelistedDomain(e).then(t => {
            let n = [];
            for (let e = 0; e < t.data.length; ++e) if (t.data[e].whitelisted_domains) for (let s = 0; s < t.data[e].whitelisted_domains.length; ++s) n.indexOf(t.data[e].whitelisted_domains[s]) < 0 && n.push(t.data[e].whitelisted_domains[s]);
            n = n.concat(s), n = MISC.uniqueArray(n);
            return callSendProfileApi(e, {
                whitelisted_domains: n
            });
        }).then(() => {
            t();
        }).catch(e => {
...
exports.callSendProfileApi = callSendProfileApi, exports.setupWhitelistedDomains = setupWhitelistedDomains;